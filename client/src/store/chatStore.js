...
const useChatStore = create((set, get) => ({
    ...
    selectChannel: async (channel) => {
        set({ currentChannel: channel, selectedUser: null, messages: [], page: 1, hasMoreMessages: true });
        const { socket } = get();
        if (socket) {
    ...
    selectUser: async (user) => {
        set({ selectedUser: user, currentChannel: null, messages: [], page: 1, hasMoreMessages: true });
    ...
    fetchMessages: async (channelId, page = 1) => {
        ...
        try {
            const response = await api.get(`/messages/${channelId}?page=${page}&limit=50`);
    ...
    fetchDirectMessages: async (userId, page = 1) => {
        if (page === 1) set({ isLoading: true });
        try {
            const response = await api.get(`/messages/direct/${userId}?page=${page}&limit=50`);
            const newMessages = response.data.messages;
    ...
    loadMoreMessages: async () => {
        const { currentChannel, selectedUser, page, hasMoreMessages, isLoading } = get();
        if ((!currentChannel && !selectedUser) || !hasMoreMessages || isLoading) return;
    ...
    sendMessage: (content) => {
        ...
        if (socket) {
            if (currentChannel) {
                ...
                socket.emit('send_direct_message', { recipientId: selectedUser.id, content });
            }
        }
    },
        ...
        try {
            ...
            set((state) => {
                if (state.onlineUsers.size === 0) {
                    ...
                    const onlineUsers = new Set(users.filter(u => u.is_online).map(u => u.id));
